...
public class CakeAPI
{
	...
	public static TextComponent getItemAsTextComponent(ItemStack is)
	{
		...
		TextComponent item = new TextComponent("§f" + CakeAPI.getItemName(is) + (is.getAmount() > 1 ? CakeAPI.recodeColorCodes(" &7(" + is.getAmount()) + ")" : ""));
		ComponentBuilder base = new ComponentBuilder(CakeAPI.getItemName(is));
		...
		if (enchs.size() > 0)
		{
			...
			for (int i = 0; i < eList.size(); i++)
				base.append(CakeAPI.recodeColorCodes("\n" + "&7" + CakeAPI.getEnchantmentName(eList.get(i)) + " " + CakeAPI.convertToRoman(eValues.get(i))));
		}
		if (lore.size() > 0)
 ...
 //		CakeAPI.spawnParticle(EnumParticle.FLAME, player.getLocation(), 128, player, 99999, 0);
	...
	public static int getPotionEffectAmplifier(Player p, PotionEffectType type)
	{
